...
| Early access is a request, not a signup | **The workspace trio is the approval** | Registering asks for early access; it does not grant it. An account becomes usable only when `workspace_username`, `workspace_email` and `workspace_address` are all set on the user document, which the server and admins can write and the person cannot. There is no separate approved flag to drift out of step with those three. Approval starts a one-week promotional trial carrying $5 of AI credit; when it ends the workspace asks to be paid for. Every one of these states is *derived* from the document on read, never stored, so it cannot go stale |
