...
| The signed-in shell | **`/account`, sidebar, no shader** | `/login` keeps `AmbientShader`, because it is still a welcome surface. `/account` does not: the shader is one fixed full-viewport WebGL canvas and behind a dense, card-heavy dashboard it fights the content and holds a GPU context on every navigation. `/account` is flat `bg-paper` with a left rail. Everything else is inherited — same hues, same wordmark, same two faces, `Reveal` and nothing louder. Neither route is indexed and neither is in the sitemap, the same treatment `/design` gets. The marketing header shows one green door: **Get Started → `/login/`** when signed out, and **Home → `/account/`** when signed in — never both Sign in and Get Started, which were the same destination |
| Age and legal data | **18+, asked at registration** | Allr is strictly an above-contract-age product. A required “I am 18 years or older” checkbox is asked in the first-time registration form, not at checkout, and an unticked answer is refused twice: by the form and again by the server validator, so a forged client gets nowhere. The attestation (`confirmedOver18`) is stored, because that stored flag is the evidence the age assertion rests on. Registration otherwise collects only the legal minimum needed to sell to someone later — legal name and country of residence. Accounts are **individual only**; there is no business signup. Residents of the **EU and the UK are not offered accounts**: those countries are left out of `src/lib/countries.ts`, which the server validates against, so the Privacy Policy's statement that Allr is not directed at them stays true. **Postal address and tax ID are deliberately deferred to checkout** and are never asked at signup. Age, Terms, and Privacy are three separate required ticks and the marketing opt-in is a fourth, unticked one: bundling consent is exactly what makes it invalid |
