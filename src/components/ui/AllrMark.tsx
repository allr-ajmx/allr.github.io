...
export function AllrMark({
  ...
  instant?: boolean;
  /** Petals accept pointer focus and report index / null. */
  interactive?: boolean;
  onPetalFocus?: (index: number | null) => void;
  className?: string;
}) {
  ...
  return (
    <svg
      ...
      className={cx(
        "mark",
        highlight !== undefined && "mark--focus",
        interactive && "mark--interactive",
        className,
      )}
      style={highlight !== undefined ? ({ ["--focus" as string]: highlight } as React.CSSProperties) : undefined}
      onPointerLeave={interactive ? () => onPetalFocus?.(null) : undefined}
    >
        ...
        >
          {PETAL_PATHS.map((petal) => (
            <path
              ...
              data-petal={petal.n}
              style={{ ["--n" as string]: petal.n }}
              fill={petal.fill}
              d={petal.d}
              onPointerEnter={
                interactive ? () => onPetalFocus?.(petal.n) : undefined
              }
            />
          ))}
        </g>
