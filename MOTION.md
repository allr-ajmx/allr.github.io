...
One direction, left to right. Words on the left. On the right (`HostingScene.tsx`): a **globe made of tiny petals** — four rings of them, each turning at its own speed and direction, so the internet looks alive — with the six petals standing inside it as **servers** (petal shape in its colour, three rack slots, a blinking LED) around a small `allr` disc. Wires leave the globe to four devices stacked on the right — phone, laptop, tablet, desktop — and **petals themselves travel the wires** (SMIL `animateMotion`, `rotate="auto"`), each landing on its device as the screen lights with the same live site. A *You · deploy v4* chip above feeds a honey petal down into the globe. Hosting is ongoing, so it loops slowly; under reduced motion nothing flies and every screen is lit. Below `lg` the stage keeps a 520px minimum inside a horizontal scroller.
